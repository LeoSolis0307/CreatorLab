...
function normalizeImages(images) {
  ...
  return arr
    .map((x) => String(x || '').trim())
    .filter((x) => x.startsWith('/img/') || /^https?:\/\//i.test(x))
    .slice(0, 12);
}
...
async function addSubmission(input) {
  ...
  if (kv) {
    ...
    await kv.set(key, arr.slice(0, 50));
...
async function listSubmissions(taskId, limit = 20, weekId = '1') {
  ...
  if (kv) {
    ...
    // Fallback: some older versions stored data as a Redis list.
    if (items.length === 0) {
      let rawItems = [];
      try {
    ...
    return { ok: true, weekId: w, taskId: t, items: items.slice(0, lim), storage: 'kv' };
  }
